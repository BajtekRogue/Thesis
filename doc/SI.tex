...
A zatem z \Cref{fact:absorbing_markov} mamy $\mathbb{P}[Z<\infty]=1$.
...
Czas całkowitego zainfekowania grafu to $Z = \max\{X_1,X_2,\dots,X_n\} = X_n$.  
Zatem rozkład zmiennej $Z$ jest już nam znany,
\[
    Z\sim\mathrm{NegBin}(n-1,p),
\]
a wartość oczekiwana wynosi
\[
...
\[
    X_v \sim \mathrm{Geo}(p),
\] 
oraz
\[
    \mathbb{E}[X_v] = \frac{1}{p}.
\]
...
Skoro $\mathbb{P}[X_v \le t] = 1 - q^t$ to mamy
\[
...
Mamy więc $Z=\max_{v\in V} X_v = X_{\frac{n}{2}}$.
Stąd intuicyjny wynik
\[
...
oraz
\[
    \mathbb{E}[X_v] = \frac{\mathrm{d}_v}{p}.
\]
...
W celu wyznaczenia rozkładu zmiennej $Y_t$ dla danej rodziny ustalamy $n=1000$ oraz wybieramy $t$ zależne od konkretnej rodziny.
Natomiast dla wartości oczekiwanej $Y_t$ przy tym samym $n$ będziemy zwiększać wartość $t$ od $1$ do wartości około $\mathbb{E}[Z]$, czyli przewidywanego czasu całkowitej infekcji.
Dla zmiennej $Z$ jej rozkład również wyznaczymy na grafie wielkości $n=1000$, a wartość oczekiwaną dla liczby wierzchołków należących do $\{1,2,\dots,1000\}$.
Na jednym wykresie naniesiemy również teoretyczne rozkłady tych zmiennych oraz wartości oczekiwanych.


Na początku oczywiście analizujemy grafy ścieżkowe.
Wybierzmy $t=np$ dla analizy rozkładu $Y_t$.
Jak widzimy na \Cref{fig}
Teraz przyjrzyjmy się grafom $\mathrm{S}_n$.

Następnie rozważmy grafy cykliczne.

Jeśli chodzi o rodzinę $\mathrm{K}_n$ to wystarczy nam empiryczna wartość $\mathbb{E}[Z]$.
Wyznaczmy ją dla każdego $n\in\{1,2,\ldots,1000\}$.
Jak widzimy na \cref{fig:complete_Z} wartości te zbiegają do liczby $2$, tak jak przewiduje teoria.
...
%         \centering
%         \includegraphics[width=\textwidth]{../img/SI/path/Yt_distribution.png}
%         % \caption{Rozkład zmiennej $Y_t$.}
%     \end{subfigure}
%     \hfill
%     \begin{subfigure}{0.48\textwidth}
%         \centering
%         \includegraphics[width=\textwidth]{../img/SI/path/Yt_expectation.png}
%         % \caption{Wartość oczekiwana $Y_t$.}
%     \end{subfigure}
%     \label{fig:path_Yt}
%     \caption{Rozkład i wartość oczekiwana zmiennej $Y_t$ dla $\mathrm{P}_n$.}
% \end{figure}
...
%     \begin{subfigure}{0.48\textwidth}
%         \centering
%         \includegraphics[width=\textwidth]{../img/SI/path/Z_distribution.png}
...
\begin{figure}[ht!]
    ...
    \begin{subfigure}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../img/SI/star/Yt_distribution.png}
        % \caption{Rozkład zmiennej $Y_t$.}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.48\textwidth}
...
\begin{figure}[ht!]
    ...
    \begin{subfigure}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../img/SI/star/Z_distribution.png}
        % \caption{Rozkład zmiennej $Z$.}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.48\textwidth}
...
\begin{figure}[ht!]
    ...
    \begin{subfigure}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../img/SI/cycle/Yt_distribution.png}
        % \caption{Rozkład zmiennej $Y_t$.}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.48\textwidth}
    ...
    \end{subfigure}
    \label{fig:cycle_Yt}
    \caption{Rozkład i wartość oczekiwana zmiennej $Y_t$ dla $\mathrm{C}_n$.}
\end{figure}
...
\begin{figure}[ht!]
    ...
    \begin{subfigure}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../img/SI/cycle/Z_distribution.png}
        % \caption{Rozkład zmiennej $Z$.}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.48\textwidth}
    ...
    \end{subfigure}
    \label{fig:cycle_Z}
    \caption{Rozkład i wartość oczekiwana zmiennej $Z$ dla $\mathrm{C}_n$.}
\end{figure}


\begin{figure}[!ht]
    \centering
    \includegraphics[width=1\textwidth]{../img/SI/complete/Z_expectation.png}
