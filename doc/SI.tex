...
Przejście ze stanu $\mathcal{I}_t=\{u\}$ do $\mathcal{I}_{t+1}=\{u,v\}$ następuje z prawdopodobieństwem $p$ w każdej jednostce czasu. 
Zatem czas zarażenia drugiego wierzchołka $X_v$ ma rozkład geometryczny, $X_v \sim \mathrm{Geo}(p)$, co za tym idzie $\mathbb{E}[X_v]=\frac{1}{p}$.

Rozważmy teraz rozkład $Y_t$. 
Mamy $\mathbb{P}[Y_t=1]=q^t$, ponieważ próba zarażenia musiałaby nie udać się $t$ razy.
Ponadto $\mathbb{P}[Y_t=2]=1-q^t$. 
Stąd $\mathbb{E}[Y_t]= 2-q^t$.

Jeśli chodzi o zmienną $Z$, to mamy $Z=X_v$, a więc również $Z\sim \mathrm{Geo}(p)$ oraz $\mathbb{E}[Z]=\frac{1}{p}$.


\section{Trójkąt}

Przyjrzyjmy się teraz nieco większemu grafowi — trójkątowi. 
Niech jeden z wierzchołków będzie źródłem $s$, a pozostałe oznaczmy $u, v$. 
Zmienna $X_v$ ma taki sam rozkład jak $X_u$.
...
\]
Policzmy teraz wartość oczekiwaną.
\begin{equation*}
\begin{aligned}
    \mathbb{E}[X_u]
    &=  \sum_{t=0}^{\infty} \mathbb{P}[X_u>t] 
    = \sum_{t=0}^{\infty} 1 - \mathbb{P}[X_u\le t] 
    = \sum_{t=0}^{\infty} q^{2t} + tpq^{2t-1} \\
    &=\frac{1}{1-q^2} +\frac{p}{q}\frac{q^2}{{(1-q^2)}^2}
    =\frac{1-q^2+pq}{{(1-q^2)}^2} = \frac{3-2p}{p{(2-p)}^2}.
\end{aligned}
\end{equation*}

Jeśli chodzi o liczbę zainfekowanych wierzchołków po $t$ krokach, to mamy trzy wartości do policzenia. 
Oczywiście $\mathbb{P}[Y_t=1]=q^{2t}$. 
Aby po $t$ chwilach tylko dwa węzły były zainfekowane, musimy zarazić któryś z wierzchołków po $1\le k\le t$ rundach z prawdopodobieństwem $2pq\cdot q^{2\cdot(k-1)}$, a następnie uzyskać $t-k$ podwójnych porażek. 
Na każdą z nich mamy szansę równą $q^2$. 
A zatem
\[
...
\begin{aligned}
    \mathbb{E}[Z]
    &=\sum_{t=0}^{\infty} \mathbb{P}[Z>t]
    =\sum_{t=0}^{\infty} q^{2t}+2tpq^{2t-1} \\
    &=\frac{1}{1-q^2}+\frac{2p}{q}\cdot \frac{q^2}{{(1-q^2)}^2}
    =\frac{-3q^2+2q+1}{{(1-q^2)}^2}
    =\frac{4-3p}{p{(2-p)}^2}.
\end{aligned}
\end{equation*}
Wykonaliśmy sporo obliczeń, jak na tak mały graf. 
...
Co za tym idzie, cały graf znajdzie się w stanie $I$ oraz $Z<\infty$ z prawdopodobieństwem $1$.
...
Jeśli $\ell = \max\{\mathrm{d}(u,v) : u\in\mathcal{I}_t, \; v\in\mathcal{S}_t\}$ to po $\ell$ rundach możemy przejść do stanu całkowitej infekcji grafu z dodatnim prawdopodobieństwem.
Zmienna $Z$ to dokładnie czas, w którym propagacja osiągnie ten stan absorbujący.
A zatem z lematu Borela-Cantellego mamy $\mathbb{P}[Z<\infty]=1$.
...
Z Twierdzenia Lebesgue'a o zbieżności ograniczonej mamy 
\[
...
\section{Grafy ścieżkowe}\label{sec:path}

Jako pierwszą rodzinę grafów rozważmy grafy ścieżkowe $\mathrm{P}_n$, jako uogólnienie pierwszego przypadku, który analizowaliśmy na samym początku.
...
\[
    X_1 = 0, \quad X_{v} = X_{v-1} + G_v, \quad v\in\{2,3,\dots,n\},
\]
gdzie $G_2,G_3,\dots,G_n \sim \mathrm{Geo}(p)$ oraz $G_2,G_3,\dots,G_n$ są niezależne.  
Widzimy zatem, że $X_v = G_2 + G_3 + \cdots + G_{v}$, a więc z \Cref{fact:sum_of_geo_RV}, $X_v$ ma rozkład ujemny dwumianowy:
\[
    X_v\sim \mathrm{NegBin}(v-1, p), \quad \mathbb{E}[X_v] = \frac{v-1}{p}.
\]

Ustalmy $t\in\mathbb{N}$ i przejdźmy do obliczania rozkładu $Y_t$.  
Zauważmy, że liczba dodatkowych zakażeń, poza startowym wierzchołkiem, do czasu $t$ to po prostu liczba sukcesów w $t$ niezależnych próbach Bernoulliego.  
...
\begin{aligned}
    ...
    &= \sum_{k=1}^{n-1} k \cdot \binom{t}{k-1} p^{k-1} q^{t-k+1} + n \cdot \sum_{j=n-1}^{t} \binom{t}{j} p^j q^{t-j} \\
...
\begin{equation*}
\begin{aligned}
    \mathbb{E}[Y_t] &= \sum_{j=0}^{t} (1+j)\binom{t}{j} p^j q^{t-j} 
    = \sum_{j=0}^{t} \binom{t}{j} p^j q^{t-j} + \sum_{j=0}^{t} j \binom{t}{j} p^j q^{t-j} \\
    &= {(p+q)}^t + t p {(p+q)}^{t-1} 
    = 1 + t p.
\end{aligned}
\end{equation*}
...
Propagacja rozchodzi się tutaj po każdym ramieniu gwiazdy niezależnie, a więc zmienne $X_1,\dots,X_n$ są niezależne.
Stąd dla każdego $v \in \{1, \dots, n\}$ mamy
\[
    X_v \sim \mathrm{Geo}(p), \quad \mathbb{E}[X_v] = \frac{1}{p}.
\] 

Kwestia zmiennej $Y_t$ jest również prosta. Ponieważ propagacja działa niezależnie na każdym wierzchołku, $Y_t$ odpowiada liczbie sukcesów w $n$ próbach Bernoulliego. Sukcesem pojedynczej próby jest zdarzenie, że zmienna $X_v$ osiągnie sukces w czasie co najwyżej $t$. 
Mamy $\mathbb{P}[X_v \le t] = 1 - q^t$, a więc prawdopodobieństwo tej próby wynosi $1-q^t$.
...
\begin{aligned}
    ...
    = \sum_{k=0}^{\infty} 1 - \mathbb{P}[Z \le k] 
    =\sum_{k=0}^{\infty} 1 - {(1 - q^k)}^n  \\[2mm]
    &= \sum_{k=0}^{\infty} \Big( 1 - \sum_{j=0}^{n} \binom{n}{j} {(-1)}^j q^{kj} \Big) 
    = \sum_{k=0}^{\infty} \sum_{j=1}^{n} \binom{n}{j} {(-1)}^{j+1} q^{kj} \\[2mm]
...
\]
to z \Cref{inequality:approximation_of_sum_by_an_integral} możemy przybliżyć tę sumę całką.  
...
\[
    ...
    = \frac{1}{\lambda} \int_{0}^{1} \sum_{j=0}^{n-1} u^j \, \mathrm{d}u = \frac{1}{\lambda}\sum_{j=0}^{n-1} \int_{0}^{1} u^j \, \mathrm{d}u = \frac{1}{\lambda} \sum_{j=0}^{n-1} \frac{1}{j+1}
    = \frac{H_n}{\lambda}.
\]
...
\[
    \mathbb{E}[Z] \sim \frac{H_n}{\log\big(\frac{1}{1-p}\big)} \sim \frac{\log(n)}{p}.
\]
...
Niech $G = (V, E)$ będzie grafem spójnym, a $G' = (V, E')$ będzie dowolnym podgrafem $G$.  
Załóżmy, że $\mathbf{X}$ opisuje proces stochastyczny w modelu SI prowadzony równocześnie na $G$ oraz $G'$ z tym samym źródłem $s \in V$, przy czym obydwa procesy są skonstruowane na wspólnej przestrzeni probabilistycznej i korzystają z tej samej losowości infekcji.  
...
Niech $G = (V, E)$ będzie grafem o $n$ wierzchołkach oraz $s \in V$ będzie ustalonym źródłem.
...
Dalej zdefiniujmy zmienne losowe $T_j = \min\{t \in \mathbb{N} : A_j \subseteq \mathcal{I}_t\}$.
...
Wtedy proces propagacji na $G'$ jest izomorficzny z procesem na grafie gwiezdnym $\mathrm{S}_{a_j}$, gdzie $a_j = |A_j|$.  
...
\begin{proof}
Korzystamy z założeń i oznaczeń, których używa \cref{lemma:helper_lemma}.
Mamy oczywiście $a_0 = 1$, a więc $a_1 + \cdots + a_h = n - 1$. 
Ponadto $T_h = U_1 + \cdots + U_h$ oraz $Z=T_h$.
Otrzymujemy zatem
\begin{equation*}
\begin{aligned}
    \mathbb{E}[Z] 
    ...
    &= h + \frac{h}{\lambda}  + \frac{1}{\lambda}\sum_{j=1}^{h} \log(a_j) 
    = h + \frac{h}{\lambda} + \frac{1}{\lambda} \log \Big(\prod_{j=1}^{h} a_j\Big) \\
    &\le h + \frac{h}{\lambda} + \frac{h}{\lambda} \log \Big(\frac{1}{h} \sum_{j=1}^{h} a_j \Big) 
    = h + \frac{h}{\lambda} \cdot \Big(1 + \log \Big(\frac{n-1}{h}\Big) \Big), \\
\end{aligned}
\end{equation*}
gdzie korzystamy kolejno z \Cref{lemma:helper_lemma} oraz \Cref{inequality:harmonic_upper_bound} i \ref{inequality:AM_GM}.
\end{proof}
...
Analizując grafy pełne przekonamy się, że dla grafów gęstych oszacowanie to nie będzie skuteczne.
...
Oczywiście $a\le b$. 
Od źródła do tego wierzchołka są dwie ścieżki: jedna o długości $a$, druga o długości $b$. 
Propagacja rozchodzi się po nich równolegle i niezależnie. 
Dla $j\in \{1,2,\dots,n-1\} $ połóżmy $U_j\sim \mathrm{NegBin}(j,p)$. 
Zmienne te są niezależne. 
Mamy wtedy 
\[
    X_v = \min\{U_a, U_b\}.
\]
Niech $F_j(t)$ będzie dystrybuantą zmiennej $U_j$. 
...
Przypomnijmy, że $\mathbb{E}[U_j]=\frac{j}{p},\; \mathrm{Var}[U_j]=\frac{jq}{p^2}$.
Z centralnego twierdzenia granicznego możemy aproksymować (\cref{fact:CLT}) $U_j \approx N_j$ gdzie $N_j\sim \mathcal{N}(\frac{j}{p},\frac{jq}{p^2})$.
Kładziemy $\mu_a=\frac{a}{p},\;\sigma^2_a=\frac{aq}{p^2}, \; \mu_b=\frac{b}{p},\; \sigma^2_b=\frac{bq}{p^2}$.
Zatem mamy $X_v \approx \min\{N_a,N_b\}$ oraz
\begin{equation*}
\begin{aligned}
    \mathbb{E}[X_v]
    &\approx \mathbb{E}[\min\{N_a,N_b\}]
    =\mathbb{E}\left[\frac{N_a+N_b-|N_a-N_b|}{2}\right] \\ 
    &=\frac{\mathbb{E}[N_a]+\mathbb{E}[N_b]-\mathbb{E}[|N_a-N_b|]}{2}
    =\frac{1}{2}\cdot(\mu_a+\mu_b-\mathbb{E}[D]),
\end{aligned}
\end{equation*}
gdzie $D=N_a-N_b$. 
Korzystając z \Cref{fact:sum_of_normal_RV} mamy $D \sim \mathcal{N}(\mu_a-\mu_b,\sigma_a^2+\sigma_b^2)$. 
...
Wtedy
\[
    \mathbb{E}[|X|] = 2\sigma\cdot \varphi\Big(\frac{\mu}{\sigma}\Big)+\mu\cdot \Big(2\mathbf{\Phi}\Big(\frac{\mu}{\sigma}\Big)-1\Big).
\]    
\end{lemma}
\begin{proof}
\[
...
Wtedy $x=\mu+\sigma z$, $\mathrm{d}{x}=\sigma \mathrm{d}{z}$. 
...
Z \Cref{lemma:abs_normal_E} dostajemy $\mathbb{E}[D]=2\xi\cdot \varphi\Big(\frac{\eta}{\xi}\Big)+\eta\cdot (2\mathbf{\Phi}\Big(\frac{\eta}{\xi}\Big)-1)$. 
Ostatecznie 
\begin{align*}
    \mathbb{E}[X_v] \approx {} & \frac{1}{2}\!\Big(\mu_a+\mu_b-2\xi\,\varphi\!\Big(\frac{\eta}{\xi}\Big)-\eta\!\Big(2\mathbf{\Phi}\!\Big(\frac{\eta}{\xi}\Big)-1\Big)\!\Big) \\[0.4em]
    ={} &\frac{\mu_a+\mu_b}{2}-\xi\,\varphi\!\Big(\frac{\eta}{\xi}\Big)-(\mu_a-\mu_b)\!\Big(\mathbf{\Phi}\!\Big(\frac{\eta}{\xi}\Big)-\frac{1}{2}\Big) \\[0.4em]
    ={} &\mu_a\!\Big(1-\mathbf{\Phi}\!\Big(\frac{\eta}{\xi}\Big)\Big)+\mu_b\,\mathbf{\Phi}\!\Big(\frac{\eta}{\xi}\Big)-\eta\,\varphi\!\Big(\frac{\eta}{\xi}\Big).
\end{align*}
...
Wtedy też $\varphi(\frac{\eta}{\xi})\to 0$ oraz $\mathbf{\Phi}(\frac{\eta}{\xi})\to 0$, a więc $\mathbb{E}[X_v] \to \mu_a = \frac{a}{p}$. 
Natomiast gdy $a=b$, to $r=\frac{1}{2}$ i $\frac{\eta}{\xi}=0$. 
Wiemy, że $\varphi(0)=\frac{1}{\sqrt{2\pi}}$ oraz $\mathbf{\Phi}(0)=\frac{1}{2}$. 
Podstawiając, otrzymamy $\mathbb{E}[X_v] \to \frac{n}{2p}-\frac{\sqrt{np}}{p\sqrt{2\pi}}$. 
Podsumowując — dla dużych $n$ mamy następujący wynik:
\[
...
Z wcześniejszego wyniku dla grafów ścieżek (\ref{sec:path}) dostajemy
\[
...
\begin{equation*}
\begin{aligned}
    & \sum_{t=0}^{\infty}\binom{2t}{j} q^{2t} 
    = \frac{1}{2} \left(\sum_{t=0}^{\infty} \binom{t}{j} q^t+ \sum_{t=0}^{\infty}\binom{t}{j} {(-q)}^t \right) \\
    &= \frac{1}{2} \left( \frac{q^j}{{(1-q)}^{j+1}} + \frac{{(-q)}^j}{{(1+q)}^{j+1}} \right) 
    = \frac{q^j}{2} \cdot \left(p^{-(j+1)}+\frac{{(-1)}^j}{{(2-p)}^{j+1}}\right).
\end{aligned}
\end{equation*}
Dalej
\begin{equation*}
\begin{aligned}
    \mathbb{E}[Z] 
    &= \sum_{j=0}^{n-2} p^j q^{-j} \frac{q^j}{2} \cdot \left(p^{-(j+1)}+\frac{{(-1)}^j}{{(2-p)}^{j+1}}\right) 
    = \frac{1}{2}\sum_{j=0}^{n-2} \frac{1}{p} + \frac{{(-p)}^j}{{(2-p)}^{j+1}} \\
    &= \frac{n-1}{2p} + \frac{1}{2(2-p)} \sum_{j=0}^{n-2} {\Big(\frac{-p}{2-p}\Big)}^j 
    = \frac{n-1}{2p} + \frac{1}{2(2-p)} \cdot \frac{1 - {(\frac{-p}{2-p})}^{n-1}}{1 - \frac{-p}{2-p}}\\
...
Jeśli $n=3$, to wynik ten pokrywa się ze wzorem uzyskanym dla grafu trójkątnego.


\section{Grafy pełne}

Graf pełny $\mathrm{K}_n$ intuicyjnie powinien mieć najszybszą propagację, bo przecież posiada maksymalną liczbę krawędzi. 
Za źródło możemy przyjąć dowolny wierzchołek $s\in V$ ze względu na symetrię grafu. 
...
Zauważmy, że jeśli $Y_1=a$, to rozkład zmiennej $Y_2$ wynosi $Y_2 = a + B$ dla $B \sim \mathrm{Bin}(n-a, 1-q^a)$. 
Zatem 
\[
    \mathbb{E}[Y_2\mid Y_1 = a] =a +(n-a)(1-q^a)= n\cdot (1-q^a) + aq^a.
\]
...
Żeby to zrobić, najpierw wyznaczmy asymptotykę $\mathbb{E}[Y_2]$. 
Oznaczamy $Y_1=1+U$, gdzie $U\sim\mathrm{Bin}(n-1,p)$. 
Z prawa całkowitej wartości oczekiwanej mamy
\[
    \mathbb{E}[Y_2] = n\cdot (1-\mathbb{E}[q^{Y_1}])+\mathbb{E}[Y_1q^{Y_1}].
\]
Musimy wyznaczyć $\mathbb{E}[q^{Y_1}]$ jak i $\mathbb{E}[Y_1q^{Y_1}]$. 
Korzystając z \Cref{fact:pgf_bin}, dostajemy 
\[
    \mathbb{E}[q^{Y_1}]=\mathbb{E}[q^{1+U}]=q\cdot \mathbb{E}[q^U]=q{(q+pq)}^{n-1}=q^n{(1+p)}^{n-1}.
\]
...
\begin{aligned}
    \mathbb{E}[Y_1q^{Y_1}] 
    &= \mathbb{E}[(1+U)q^{1+U}] 
    = q(\mathbb{E}[q^U]+\mathbb{E}[Uq^U]) \\
    &= q(q^{n-1}{(1+p)}^{n-1}+(n-1)pq^{n-1}{(1+p)}^{n-2}) \\
    &= q^n{(1+p)}^{n-2}(1+p+(n-1)p)=q^n{(1+p)}^{n-2}(1+np).
\end{aligned}
\end{equation*}
Podstawiając uzyskane przed chwilą wyrażenia do wzoru na $\mathbb{E}[Y_2]$ dostaniemy
\begin{equation*}
\begin{aligned}
    \mathbb{E}[Y_2] 
...
Korzystając z nierówności Markova (\ref{inequality:Markov}) otrzymujemy $\mathbb{P}[Z\ge 3]=\mathbb{P}[n-Y_2\ge 1]\le \mathbb{E}[n-Y_2]=\varepsilon_n$. 
Dalej zauważmy, że $\mathbb{P}[Z=1]=p^{n-1}$, bo wszystkie próby zarażenia w rundzie pierwszej musiałby się powieść. 
...
\begin{aligned}
    ...
    = \mathbb{P}[Z\ge 1] + \mathbb{P}[Z\ge 2] + \sum_{k = 3}^{\infty} \mathbb{P}[Z\ge k]  \\
    &= 1 + 1-p^{n-1} + \mathbb{E}[Z \cdot \mathbf{1}_{Z\ge 3}] 
    \le 2 - p^{n-1} + \sqrt{\mathbb{E}[Z^2]}\sqrt{\mathbb{E}[\mathbf{1}_{Z\ge 3}]} \\
...
\begin{proof}
Mamy $Y_t=|\{v\in V: X_v \le t\}|$ zatem $Y_t=\sum_{v\in V}  \mathbf{1}_{X_v\le t}$. Nakładając na tę równość operator $\mathbb{E}$ otrzymujemy
\[
...
Przejdźmy teraz do obliczania średniej liczby zainfekowanych wierzchołków w czasie $t$. 
Oznaczmy przez $F(t;m,p)$ dystrybuantę zmiennej o rozkładzie $\mathrm{NegBin}(m,p)$. 
...
\]
Ponadto, gdy $t<j\le h$, to $F(t; j, p)=0$, bo żaden wierzchołek w odległości od korzenia większej niż liczba rund, nie może zostać zarażony. 
Możemy zatem zmniejszyć granice sumowania: 
\[
    \mathbb{E}[Y_t] = \sum_{j=0}^{\min\{h,t\}} a_j\cdot F(t; j, p).
\]

Oszacujmy teraz średni czas całkowity czas propagacji drzewa.
Niech $L$ będzie zbiorem liści w $G$. 
Wtedy mamy $Z = \max_{u\in L} X_{u}$.
Zauważmy, że $h = \max_{u\in L} d_{u}$. 
Z nierówności Jensena (\ref{inequality:Jensen}) otrzymujemy
\[
    \mathbb{E}[Z]=\mathbb{E}\Big[\max_{u\in L} X_{u}\Big] \ge \max_{u\in L} \mathbb{E}[X_{u}] = \max_{u\in L} \frac{d_{u}}{p} = \frac{h}{p}.
\]
...
Sam lemat daje nierówność, której nie da się poprawić, co wiemy poprzez analizę dla grafów gwiazd. 
...
Widzimy zatem, że nasze ograniczenia nie są do poprawienia bez znajomości dodatkowych parametrów drzewa. 
...
Wyznaczmy rozkład dla wartości $t=r\cdot\frac{n}{p}$ dla $r\in\{0.1, 0.5, 0.8, 0.9\}$.
Widzimy, że rozkłady są coraz bardziej skoncentrowane na lewo, zgodnie z przewidywaniami (patrz \cref{fig:SI_path_Yt}).
...
Również wartości $\mathbb{E}[Z]$ są zbliżone to teoretycznej asymptotyki (\cref{fig:SI_star_Z}).
...
Aby się upewnić, że rzeczywiście tak jest, sprawdźmy to dla $p\in\{0.2, 0.1\}$.
Jak widać na \Cref{fig:SI_complete_Z}, dla większych $n$ czas całkowitego poinformowania grafu zbiega do $2$.
