...
Z wcześniejszego wyniku dla grafów ścieżek dla $n\ge 1 + 2t$ dostajemy
\[
    \mathbb{E}[Y_t] = 1+2tp.
\]
...
Widzimy, że rozkłady są sa coraz bardziej skoncentrowane na lewo, zgodnie z przewidywaniami (patrz \cref{fig:path_Yt}).
Nie będziemy już wyznaczać wartości oczekiwanej $Y_t$.
Dla $Z$ sprawa jest prosta. 
Rozkład i wartość oczekiwana przedstawiono na \Cref{fig:path_Z}.

Teraz przyjrzyjmy się grafom $\mathrm{S}_n$.
Dla rozkładu $Y_t$ ustalmy $t=\lfloor \frac{\log(n)}{2p} \rfloor$.
Widzimy, że wyniki niemal idealnie pokrywają się z oczekiwaniami teoretycznymi dla grafów gwiezdnych. (patrz \cref{fig:star_Yt}).
Również wartości $\mathbb{E}[Z]$ są zbliżone to teoretycznej asymptotyki, to jest $\frac{H_n}{\lambda}, \; \lambda=\log(\frac{1}{1-p})$ (\cref{fig:star_Z}).
...
Wyniki te przedstawiono na \Cref{fig:cycle_Z}.
...
Jak widać na \Cref{fig:complete_Z} rzeczywiście dla wiekszych $n$ czas całkowitego poinformowania grafu zbiega do $2$.

\begin{algorithm}
\caption{SI Propagation}
\begin{algorithmic}[1]
    \State \textbf{Input:} Graph $G=(V,E)$, infection probability $p$, source $s\in V$
    \State \textbf{Output:} Sets of infected vertices $(\mathcal{I}_t)$ and final time $Z$
    \State $\mathcal{I}_0 \gets \{s\}$
    \State $t \gets 0$

    \While{$|\mathcal{I}_t| < |V|$}
        ...
        \For{each $u \in \mathcal{I}_t$}
            ...
            \EndFor
        \EndFor
        \State $\mathcal{I}_{t+1} \gets \mathcal{I}_t \cup \mathcal{I}_{\text{new}}$
        \State $t \gets t + 1$
    \EndWhile
