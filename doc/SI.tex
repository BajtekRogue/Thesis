...
% Postarjamy się lepiej oszacować wartość oczekiwaną. Oczywiście $X_k\le N_k$ jak i $X_k\le N_{n-k}$ stąd
% \[
%     \mathbb{E}[X_k]\le\min\{\mathbb{E}[N_k], \mathbb{E}[N_{n-k}]\} = \min\left\{\frac{k}{p}, \frac{n-k}{p}\right\}=\frac{1}{p}\cdot \min\{k,n-k\}
% \]
% Zauważmy, że $\min\{k,n-k\}=\mathrm{d}(s,k)$. Okazuje się, że oczekiwany czas potrzebny do poinformowania wierzchołka w danej odległości jest mniejszy niż dla grafów ścieżkowych, co spowodowane jest istnieniem dwóch ścieżek.
...
\]
Korzystając z \Cref{F:sum_of_normal_RV} mamy $C \sim \mathcal{N}(\mu_a-\mu_b,\sigma_a^2+\sigma_b^2)$ gdzie $C=A-B$. Oznaczmy $\eta=\mu_a-\mu_b$ oraz $\xi=\sqrt{\sigma_a^2+\sigma_b^2}$. Z \Cref{F:E_abs_normal} dostajemy $\mathbb{E}[C]=2\xi\cdot \varphi\Big(\frac{\eta}{\xi}\Big)+\eta\cdot (2\mathbf{\Phi}\Big(\frac{\eta}{\xi}\Big)-1)$. Ostatecznie 
\begin{equation*}
\begin{aligned}
\mathbb{E}[X_k] \approx
