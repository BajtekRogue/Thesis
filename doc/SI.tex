\section{Dwa wierzchołki, jedna krawędź $\mathrm{P}_2$}
...
Istnieją tylko dwa możliwe stany: $\mathcal{I}_t=\{u\}$ oraz $\mathcal{I}_t=\{u,v\}$. 
...
\]
Z centralnego twierdzenia granicznego możemy aproksymować $N_a \approx A,\; N_b \approx B$.
Zatem mamy $X_v \approx \min\{A,B\}$ oraz
\[
...
\begin{aligned}
    ...
    = \frac{1}{2} \Big(\sum_{t=0}^{\infty} \binom{t}{j} q^t+ \sum_{t=0}^{\infty}\binom{t}{j} {(-q)}^t \Big) \\
    &= \frac{1}{2} \Big( \frac{q^j}{{(1-q)}^{j+1}} + \frac{{(-q)}^j}{{(1+q)}^{j+1}} \Big) 
    = \frac{q^j}{2} \cdot \Big(p^{-(j+1)}+\frac{{(-1)}^j}{{(2-p)}^{j+1}}\Big).
\end{aligned}
\end{equation*}
Dalej
\begin{equation*}
\begin{aligned}
    \mathbb{E}[Z] 
    &= \sum_{j=0}^{n-2} p^j q^{-j} \frac{q^j}{2} \cdot \Big(p^{-(j+1)}+\frac{{(-1)}^j}{{(2-p)}^{j+1}}\Big) 
