...
W modelu \textbf{SIR} symulować będziemy rozkłady zmiennych $W,Z$ oraz ich wartości oczekiwane.
...
\begin{algorithm}
\caption{SIR Propagation}
\begin{algorithmic}[1]
\State\textbf{Input:} Graph $G=(V,E)$, infection probability $p$, recovering probability $\alpha$, source $s\in V$
\State\textbf{Output:} Sets of recovered vertices $(\mathcal{R}_t)$ and extinction time $Z$
...
\While{$\mathcal{I}_t \ne \varnothing$ \textbf{and} $\exists v\in\mathcal{I}_t: \mathcal{S}_t\cap\mathrm{N}(v)\neq\varnothing$}
    \State$\mathcal{I}_{\text{new}} \gets \varnothing$
    \For{each $u \in \mathcal{I}_t$}
        \For{each $v \in \mathrm{N}(u)$}
            \If{$v \in \mathcal{S}_t$ \textbf{and} $\text{random}() < p$}
                \State$\mathcal{I}_{\text{new}} \gets \mathcal{I}_{\text{new}} \cup \{v\}$
            \EndIf%
        \EndFor%
    \EndFor%
    \State$\mathcal{R}_{\text{new}} \gets \varnothing$
    \For{each $u \in \mathcal{I}_t$}
        \If{$\text{random}() < \alpha$}
            \State$\mathcal{R}_{\text{new}} \gets \mathcal{R}_{\text{new}} \cup \{u\}$
        \EndIf%
    \EndFor%
    \State$\mathcal{S}_{t+1} \gets \mathcal{S}_t \setminus \mathcal{I}_{\text{new}}$
    \State$\mathcal{I}_{t+1} \gets (\mathcal{I}_t \cup \mathcal{I}_{\text{new}}) \setminus \mathcal{R}_{\text{new}}$
    \State$\mathcal{R}_{t+1} \gets \mathcal{R}_t \cup \mathcal{R}_{\text{new}}$
    \State$t \gets t + 1$
\EndWhile%
