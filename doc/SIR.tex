...
W celu oswojenia się z bardziej skomplikowanym modelem, jakim jest \textbf{SIR}, przeanalizujmy graf o jednej krawędzi. 
...
Jest cecha kontrastująca \textbf{SIR} względem \textbf{SI}.
...
\[
    Z\sim\mathrm{Geo}(1-q\beta).
\]
Mamy
\[
    \mathbb{E}[Z]=\frac{1}{1-q\beta}.
\]


\section{Pewne wygaśnięcie}

W modelu \textbf{SIR} wygaśnięcie infekcji jest zdarzeniem pewnym. 
...
A więc z \Cref{fact:absorbing_markov} mamy $\mathbb{P}[Z<\infty]=1$.
Zauważmy, że 
\[
...
Podobnie jak w modelu \textbf{SI} propagacja rozchodzi się miedzy kolejnymi wierzchołkami niezależnie a więc
\[
...
\[
    X_v\sim\mathrm{KNegBin}(v-1,p,\alpha),
\]
oraz
\[
    \mathbb{E}[X_v|X_v<\infty] = \frac{v-1}{1-q\beta}.
\]
...
A więc dla $k>0$ mamy
\[
    \mathbb{P}[Y_t\ge k+1] = \mathbb{P}[X_k\le t] = \sum_{j=k}^{t} \binom{j-1}{k-1}p^k (q\beta)^{j-k}.
\]
Dalej mamy
\[
...
\]
Oznaczmy $\tau = \frac{p}{1-q\beta}$.
Wtedy
\[
    \mathbb{P}[W=k]=(1-\tau)\tau^{k-1}, \quad k\in\{1,2,\dots,n-1\}, \quad \mathbb{P}[W=n]=\tau^{n-1}.
\]
Ponadto
\[
    \mathbb{E}[W] = \sum_{k=0}^{n-1} \mathbb{P}[W\ge k+1] = \sum_{k=0}^{n-1} \tau^k = \frac{1-\tau^n}{1-\tau}.
\]
...
Niech $G_j(z)=\mathbb{E}[z^{T_j}]$. 
Mamy
\[
    G_j(z) = \sum_{k=1}^{\infty} (1-q\beta)(q\beta)^{k-1}z^k=\frac{(1-q\beta)z}{1-q\beta z}.
\]
Zauważmy, że $Z=T_1+T_2+\cdots+T_W$.
A zatem
\[
    \mathbb{E}[z^Z]=\sum_{k=1}^{n-1} \mathbb{P}[W=k]\cdot G_k(z).
\]
Funkcja generująca prawdopodobieństwo determinuje rozkład $Z$.
Natomiast wartość oczekiwana wynosi
\[
    \mathbb{E}[Z]=\mathbb{E}[W]\cdot\mathbb{E}[T_1]=\frac{1-\tau^n}{1-\tau} \cdot \frac{1}{1-q\beta} = \frac{1-\tau^n}{\frac{q\alpha}{1-q\beta}} \cdot \frac{1}{1-q\beta} = \frac{1-\tau^n}{q\alpha}.
\]


\section{Grafy gwiezdne}

Rozpatrzmy propagacje na rodzinie $\mathrm{S}_n$, gdzie źródłem jest wierzchołek $0$.
...
$Y_t$ zachowuje się tak samo jak w modelu \textbf{SI} dla grafów gwiezdnych.
Przypomnijmy, że rozkład ten wynosi $1+B_{j,t}$ gdzie $B_{j,t}\sim\mathrm{Bin}(n,1-q^{\min\{j,t\}})$.
A więc
\[
    \mathbb{P}[Y_t=k+1|C=j] = \binom{n}{k}(1-q^{\min\{j,t\}})(q^{\min\{j,t\}})^{n-k}.
\]
...
\[
    \mathbb{P}[W=k+1] = \frac{\alpha}{\beta}\binom{n}{k} \sum_{j=1}^\infty (1-q^j)^k(q^j)^{n-k}\beta^j.
\]
oraz
\[
    \mathbb{E}[W]=1+\frac{np}{1-q\beta}.
\]
...
Dodatkowo skorzystaliśmy z \Cref{fact:beta_gamma}.
