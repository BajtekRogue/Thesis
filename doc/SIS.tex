...
Dla modelu \textbf{SIS} przeprowadzimy symulacje na grafie $\mathrm{K_n}$.
Na nasze potrzeby wystarczy nam $n=10$.
Aby mieć pewność, że infekcji umrze w sensownym czasie będziemy badać propagacje dla parametrów $p=0.1,\;\alpha=0.9$ (\Cref{fig:p0.1_a0.9}), $p=0.2,\;\alpha=0.8$ (\Cref{fig:p0.2_a0.8}) oraz $p=0.5,\;\alpha=0.9$ (\Cref{fig:p0.5_a0.9}).
...
Wreszcie zasymulujemy rozkład $Z$ a potem $\mathbb{E}[Z]$ dla $n\in\{1,2,\dots,10\}$.
...
\begin{algorithm}
\caption{SIS Propagation}
\begin{algorithmic}[1]
    \State \textbf{Input:} Graph $G=(V,E)$, infection probability $p$, recovering probability $\alpha$, source $s\in V$
    \State \textbf{Output:} Sets of infected vertices $(\mathcal{I}_t)$ and extinction time $Z$
    \State $\mathcal{I}_0 \gets \{s\}$
    \State $t \gets 0$

    \While{$|\mathcal{I}_t| > 0$}
        ...
        \For{each $u \in \mathcal{I}_t$}
            ...
            \EndIf
        \EndFor
        \State $\mathcal{I}_{t+1} \gets (\mathcal{I}_t \cup \mathcal{I}_{\text{new}}) \setminus R$
        \State $t \gets t + 1$
    \EndWhile
