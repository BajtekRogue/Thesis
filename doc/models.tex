...
Definiujemy realizację zmiennej losowej $\mathbf{X} : \mathbb{N}\times V \to \mathcal{Q}$ określającą stany wierzchołków grafu w czasie.
...
Dla wygody będziemy oznaczać $q=1-p$. 
...
Dla $t\in\mathbb{N}_+$ definiujemy następujące zbiory:
\[
    \mathcal{S}_t=\{v\in V: \mathbf{X}_t(v) = S\}, \quad \mathcal{I}_t=\{v\in V: \mathbf{X}_t(v) = I\}
\]
Dalej dla uproszczenia notacji kładziemy $i(t,v)=|\mathrm{N}(v) \cap \mathcal{I}_t|$.
Rozkład prawdopodobieństwa w tym modelu jest definiowany przez następujące zależności:
\begin{align*}
&\mathbb{P}[\mathbf{X}_{t+1}(v) = S \mid \mathbf{X}_t(v) = S] = q^{i(t,v)}, \\[0.3em]
&\mathbb{P}[\mathbf{X}_{t+1}(v) = I \mid \mathbf{X}_t(v) = S] = 1 - q^{i(t,v)}, \\[0.3em]
&\mathbb{P}[\mathbf{X}_{t+1}(v) = S \mid \mathbf{X}_t(v) = I] = 0, \\[0.3em]
&\mathbb{P}[\mathbf{X}_{t+1}(v) = I \mid \mathbf{X}_t(v) = I] = 1.
\end{align*}
Diagram przejść dla tego modelu ilustruje \ref{fig:diag_SI}.
...
Wyznaczenie rozkładu $X_v$ jak i $\mathbb{E}[X_v]$ dostarczy sporo informacji o propagacji na grafie w zależności od jego topologii.
...
\begin{tikzpicture}[scale=1.5, node distance=3cm]
    ...
    \draw[->, thick, loop above] (S) to node[above] {$q^{i(t,v)}$} (S);
    \draw[->, thick, loop above] (I) to node[above] {$1$} (I);
    \draw[->, thick] (S) to node[above] {$1-q^{i(t,v)}$} (I);
\end{tikzpicture}
...
\end{align*}
Diagram przejść dla tego modelu ilustruje \ref{fig:diag_SIR}.
...
Z tego powodu, niezależnie od struktury grafu, wartość oczekiwana $\mathbb{E}[X_v]=\infty$.
...
Ale transmisja ze stanu $I$ do $R$ na pojedynczym wierzchołku jest rozkładem $\mathrm{Geo}(\alpha)$, a więc zmienna ta byłaby po prostu sumą rozkładu geometrycznego oraz $X_v$.
Z tego powodu nie będziemy jej rozpatrywać. 
...
\]
Dodatkowo definiujemy zmienną $W$ będącą liczbą finalnie wyzdrowiałych wierzchołków:
\[
    W = |\{v\in V : X_v < \infty\}|.
\]
Rozkłady $Z,W$ oraz ich wartości oczekiwane będą naszym głównym obiektem zainteresowań w tym modelu.
...
Natomiast w drugiej, te zarażone wierzchołki, które były juz zarażone przed fazą pierwszą, mogą powrócić do stanu podatności.
...
\end{align*}
Diagram przejść dla tego modelu ilustruje \ref{fig:diag_SIS}.
...
Dlatego też nie będziemy się nimi zajmować.
Skupimy się tylko na rozkładzie $Y_t$.
Jeśli chodzi o zmienną $Z$, to przyjrzyjmy się zarówno jej rozkładowi, jak i jej wartości oczekiwanej.
...
\begin{tikzpicture}[scale=1.5, node distance=3.5cm]
    ...
    \draw[->, thick, loop above] (S) to node[above] {$q^{i(t,v)}$} (S);
    \draw[->, thick, loop above] (I) to node[above] {$\beta$} (I);
    \draw[->, thick, bend left=30] (S) to node[above] {$1-q^{i(t,v)}$} (I);
